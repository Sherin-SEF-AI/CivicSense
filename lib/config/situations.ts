...
/**
 * The civic situation catalogue: deployment configuration, not data.
 *
 * Each entry is a situation the platform can recognise, with the inherent
 * severity the severity function starts from, the department that owns it, and
 * the statutes counsel has cleared for selection against it. The reasoning layer
 * may only select from this list and must justify each selection; it never
 * invents a section.
 *
 * Every statute here carries a verification flag. Anything not yet confirmed by
 * counsel for the deployment state renders as reference only, and no
 * court-bound package may cite it.
 */
